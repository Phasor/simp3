...
type ChatAccess = Tables<'chat_access'>;
type ChatRules = Tables<'chat_rules'>;
 ...
 */
function coerceStatusFromApi(raw: any): ChatAccessStatus {
  // Defensive normalization; tolerate missing fields
  const hasAccess = !!raw?.hasAccess;
  const accessUntil = raw?.accessUntil ? new Date(raw.accessUntil) : null;
  const isExpired = raw?.isExpired !== undefined ? !!raw.isExpired : (accessUntil ? Date.now() > accessUntil.getTime() : true);
  const timeRemaining = typeof raw?.timeRemaining === 'number'
    ? raw.timeRemaining
    : accessUntil ? Math.max(0, accessUntil.getTime() - Date.now()) : null;
  const daysRemaining = Number.isFinite(raw?.daysRemaining)
    ? raw.daysRemaining
    : timeRemaining != null ? Math.floor(timeRemaining / 86_400_000) : null;
  const hoursRemaining = Number.isFinite(raw?.hoursRemaining)
    ? raw.hoursRemaining
    : timeRemaining != null ? Math.floor((timeRemaining % 86_400_000) / 3_600_000) : null;
  const minutesRemaining = Number.isFinite(raw?.minutesRemaining)
    ? raw.minutesRemaining
...
/**
 * Get creator's chat rules or default values
 */
export async function getCreatorChatRules(creatorId: string) {
  try {
    const supabase = createSupabaseBrowser();
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        ...
        console.log('⚠️ No chat rules found, using defaults');
      } else if (error.code === 'PGRST301' || error.code === '42501') {
        console.warn('⚠️ RLS prevented reading chat_rules; using defaults.');
      } else {
        console.error('⚠️ Error fetching chat rules:', error);
      }
      return {
        rules: getDefaultChatRules(creatorId),
        error: null // Don't treat as error, use defaults gracefully
      };
    }

    return {
      rules: data || getDefaultChatRules(creatorId),
      error: null
    };
  } catch (error) {
    console.log('⚠️ Chat rules query failed, using defaults:', error);
    return {
      rules: getDefaultChatRules(creatorId),
      error: null // Use defaults gracefully
    };
  }
}

/**
 * Get default chat rules for a creator
 */
function getDefaultChatRules(creatorId: string): ChatRules {
  return {
