...
import { checkChatAccess, getUserChatAccess, type ChatAccessValidationResult, type ChatAccessStatus } from '@/lib/chatAccess';
...
export function useUserChatAccess(): UseUserChatAccessReturn {
  ...
  const [chatAccess, setChatAccess] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
