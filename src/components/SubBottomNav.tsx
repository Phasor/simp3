...
// Routes where the bottom nav hides completely on mobile
const HIDE_ON_MOBILE = ['/chat']

// Routes where the bottom nav never shows
const ALWAYS_HIDDEN_PATTERNS = [
  /^\/task\/[^/]+\/pay$/,      // Send Moment
...
export default function SubBottomNav() {
  ...
  // Never show on Send Moment or task completion
  ...
  const isMobileChatHidden = HIDE_ON_MOBILE.some(r => pathname.startsWith(r))

  return (
    <nav
      className={`fixed bottom-0 left-0 right-0 z-50 bg-black border-t border-gray-900 safe-area-bottom ${
        isMobileChatHidden ? 'hidden md:flex' : 'flex'
      }`}
    >
