...
export default function SignupPage() {
  ...
  const signUpWithGoogle = async (type?: 'CREATOR' | 'FAN') => {
    const resolvedType = type ?? userType
    if (!resolvedType) return

    try {
      localStorage.setItem('signup_user_type', resolvedType)

      const redirectTo = `${window.location.origin}/auth/callback?next=/signup&ut=${resolvedType}`
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="max-w-4xl w-full">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          {/* Creator Card */}
          <div className="border-2 rounded-xl p-8 transition-all border-gray-200 bg-white hover:border-gray-300 hover:shadow-lg flex flex-col">
            ...
            <div className="mt-6 space-y-3">
              <button
                onClick={() => signUpWithGoogle('CREATOR')}
                className="w-full py-2.5 rounded-lg border border-gray-300 font-medium text-gray-700 hover:bg-gray-50 transition-colors flex items-center justify-center gap-3"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              ...
              <button
                ...
                className="w-full py-2.5 rounded-lg border border-gray-300 font-medium text-gray-700 hover:bg-gray-50 transition-colors text-sm"
              >
                Sign up with email
              </button>
            </div>
